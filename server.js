...
const TRACKING_CONFIG = {
  ...
  giftFormUrl: 'https://forms.gle/THAY_LINK_GOOGLE_FORM_CUA_BAN',
  ...
  milestones: [
    {
      ...
      items: ['Phục vụ stream', 'LED Hà Nội', 'LED TP.HCM', 'LED Đà Nẵng'],
    },
    {
      amount: 150000000,
      title: 'DẬY SÓNG',
      subtitle: '150.000.000 VNĐ',
      items: ['Phục vụ stream', 'LED Hà Nội', 'Roadshow xe bus Hà Nội', 'LED TP.HCM', 'LED Đà Nẵng'],
    },
    {
      amount: 200000000,
      title: 'VIỄN CHINH',
      subtitle: '200.000.000 VNĐ',
      items: ['Phục vụ stream', 'LED Hà Nội', 'Roadshow xe bus Hà Nội', 'LED TP.HCM', 'LED Đà Nẵng', 'LED Hàn Quốc'],
    },
    {
      amount: 220000000,
      title: 'KHẢI HOÀN',
      subtitle: '220.000.000 VNĐ',
      items: ['Phục vụ stream', 'LED Hà Nội', 'Roadshow xe bus Hà Nội', 'LED TP.HCM', 'LED Đà Nẵng', 'LED Hàn Quốc', 'LED Thái Lan', 'LED Nhật Bản'],
    },
  ],
};
// =========================================================
...
async function boot() {
  // 1. Mở server trước để Render nhận được PORT ngay
  app.listen(PORT, () => {
    console.log(`Cake tracker running on port ${PORT}`);
    console.log('Sửa cấu hình theo dõi trong file server.js, mục TRACKING_CONFIG.');
  });

  // 2. Load dữ liệu Excel local sau khi server đã chạy
  try {
    await loadRowsFromExcel();
  } catch (error) {
    cache.lastError = error.message || String(error);
    console.error('Lỗi load Excel:', cache.lastError);
  }

  // 3. Quét sao kê lần đầu ở background, không chặn app start
  refreshTransactions().catch((error) => {
    cache.lastError = error.message || String(error);
    console.error('Lỗi quét lần đầu:', cache.lastError);
  });
  ...
  setInterval(() => {
    refreshTransactions().catch((error) => {
      cache.lastError = error.message || String(error);
      console.error('Lỗi quét định kỳ:', cache.lastError);
    });
  }, tick);
}
